...
| ✨ NEW: AI design copilot (laser-studio) | ✅ Same chat architecture over the 2D laser engine: get_canvas (objects, bboxes, layers w/ power/speed), add_shape (rect/ellipse/polygon/star/stroke-text), add_path (presets: heart/gear/slot/arch/arrow/cross/lbracket, or custom points), edit/delete/duplicate/flip, assign_layer + set_layer, group, linear & circular patterns, job estimate. Understands y-down/top-left conventions, layer semantics (cut vs engrave), holes-as-inner-cut-paths. Live-verified: 60×40 rounded name tag + centred engraved MIA + 4mm hanging hole placed to the mm |
