...
| ✨ NEW: AI modeling copilot (cad-studio) | ✅ Chat panel ("✨ AI" button), dual provider — **Gemini (default, key pre-loaded from gitignored `local-keys.js`) or Claude** — drives the real engine via tool-use: get_scene (world bboxes), add/edit/duplicate/delete, booleans, params, mass inspect, view. Understands references ("that box", "touching the back-left corner") from live scene state; every AI edit goes through pushHist → fully undoable. Bring-your-own Anthropic API key (localStorage, direct CORS call). Tool loop verified LIVE with Gemini 2.5 Flash (plate shortened, hole re-cut, cube placed flush on the corner) and with a mocked Anthropic API; helpers unit-tested (tests/chat-tools.test.js, 21 asserts) |
